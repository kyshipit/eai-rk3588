Language: **English** | [中文](architecture-and-runtime.md)
...
> Module detail and acceptance: topic docs in [README_EN.md](../README_EN.md); implementation: `runtime/` code.
...
*Solid: frames/data; dashed: config and person/face signals. TTS modules: [tts-melotts_EN.md](tts-melotts_EN.md) §3–4; startup and threads: this doc §5–6.*
...
`SYS>` and Failed/Ready text: [llm-model-coordinator_EN.md](llm-model-coordinator_EN.md) §5. Entry: [`app/main.cc`](../runtime/app/main.cc), [`pipeline.cpp`](../runtime/engine/pipeline.cpp).
...
**Shutdown**: `Stop()` → `AbortActiveGeneration`, release camera, quit sentinels → `tts`/`llm` `Shutdown()`. Issues: [troubleshooting_EN.md](troubleshooting_EN.md).
...
| ----------------------- | ------------------------------------------------------------------- | ---------------------------------------------- |
| **Gate / greet / `YOU>`** | `LlmGreeting`: Locked→Arming→Active→Grace; `prompt_gate` needs `IsReady()` | [llm-model-coordinator_EN.md](llm-model-coordinator_EN.md) |
| **RKLLM**               | `infer_thread_` + `rkllm_run`; chunks → `PollDeferred`              | same                                           |
| **TTS**                 | Ingress → Planner → synth/play (short Static; gst idle prime after `Cancel`); when active **skip yolo infer only** | [tts-melotts_EN.md](tts-melotts_EN.md) (**acceptance**) |
| **Adapter files**       | `adapters/{yolo,scrfd,llm,tts}/`                                    | [adapters_EN.md](adapters_EN.md)                     |
...
Reading order and index: **[README_EN.md](../README_EN.md)**.
