...
| 参与板端运行 | 不参与 |
|--------------|--------|
| `runtime/` 编译产物 | 仓库根 `verify/`（PC 侧转换验证） |
...
| TTS | 已集成 `TtsWorker` + MeloTTS RKNN；FastAck 短反馈 + 正式回答规划播报 | 无语音 / 云端 TTS | 展台需扬声器反馈；`gst-launch-1.0` PCM 管道 |
| 终端 UX | `SYS>` / `YOU>` / `AI>` 分流 | 纯 GUI 聊天 | 板端串口与调试友好；窗口只管视觉 overlay |
| 配置来源 | 单文件 `default.yaml` | 代码内默认 | 板端行为可审计、可 diff |

**是否有更好方案？** 取决于产品：纯安防可 `model.llm.enabled: false`；多摄像头需扩展 `CameraSource` 与 Pipeline 输入抽象。当前架构在「单路相机 + 按需 NPU 槽 + 旁路对话」下平衡了复杂度与 RK3588 资源。
...
| TTS | 逻辑 | `model.tts.enabled`（已集成） | `TtsWorker` |
...
平台已集成 MeloTTS：`model.tts.enabled` 控制是否启用（需 `model.llm.enabled` 创建对话链路）。

```text
SetBannerLine(问候) ──► TtsWorker::PlayText（整段）
YOU> accepted ──► FastAck 缓存短反馈（≤1s）
RKLLM chunk event ──► TtsIngress（过滤 thinking）
                   ──► TtsPlanner（中/英正式回答规划）
                   ──► FormalAnswerBuffer（安全缓冲后开播）
TtsWorker（合成/播放双线程）→ MeloTTS → gst-launch PCM 管道
```

要点：

- 语音对话体验：**≤1s 短反馈 + 正式回答开播后 `pcm underrun=0`**（详见 TTS 主文档 §2）。
- `LlmWorker::OnLlmChunk` 仅投递 chunk event；主线程 `PollDeferred` 统一处理 TTS。
- 新 `YOU>` / `Abort` / `Stop` → `TtsWorker::Cancel`（清队列，保持 gst 管道）。
- 人脸对话 TTS 活跃期：`Pipeline` 跳过 yolo inference（`ShouldSkipYoloForDialogueTts()`），**不关闭 yolo/scrfd 槽位**，不破坏 Grace/Locked 门控。
...
| `model.tts.enabled` | TtsWorker | 是否语音播报 |
| `model.tts.qos.enable_visual_throttle` | ModelCoordinator + TtsWorker | TTS 低水位时是否临时降视觉负载 |
...
| 提问 | 框持续 | `YOU>` → 流式 `AI>` | **FastAck 短反馈 + 正式回答连续播** | `rkllm_run` + TtsPlanner |
...
3. 常见问题：Ctrl+C 不退、退出 SIGSEGV → [运行排障.md](运行排障.md)。
...
|------|------|
| `runtime/app` | 配置与启动 |
| `runtime/engine` | Pipeline、队列 |
| `runtime/platform` | ModelCoordinator、LlmGreeting |
| `runtime/adapters/yolo, scrfd, llm, tts` | 模型插件 |
| `runtime/capture, display` | 显示与采集 |

板端只认 `runtime/` + `./model/*.rknn`；仓库根 `verify/` **不参与**板端运行。

### 默认配置

见 `runtime/config/default.yaml`：`model.yolo`、`model.scrfd`、`model.llm`、`model.tts`、`system.switch.*`。
...
| idle | yolo |
| person | yolo + scrfd |
| 问候 | llm 逻辑槽：静态文案 `SetBannerLine`（须模型 Ready） |
| 语音 | tts 逻辑槽：播 `AI>` 可播正文 |

---

## 14. 后续演进方向（非验收 backlog）

- **VAD/ASR/AEC**：实现真实语音输入与 barge-in 打断
- **更快 TTS / 预合成话术**：扩展 FastAck 话术库或替换更快模型
- **YOLO-World**：开放词汇视觉哨兵
...
| 文档 | 说明 |
|------|------|
| [系统架构与运行逻辑.md](系统架构与运行逻辑.md) | 本文：平台框架、加载顺序、设计取舍 |
| [TTS与MeloTTS集成说明.md](TTS与MeloTTS集成说明.md) | **唯一** TTS/语音对话设计与验收 |
...
| [doc-readme.md](doc-readme.md) | docs 极简索引 |
