...
**是否有更好方案？** 取决于产品：纯安防可 `model.llm.enabled: false`；多摄像头需扩展 `CameraSource` 与 Pipeline 输入抽象（见 [讨论备忘-多槽与模型演进.md](讨论备忘-多槽与模型演进.md)）。当前架构在「单路相机 + 按需 NPU 槽 + 旁路对话」下平衡了复杂度与 RK3588 资源。
...
设计约定：**LLM（及可选 TTS）预加载先于 Pipeline 内 YOLO 同步 Init**，以优先占用对话模型、减少用户走近后的等待。
...
详见 [LLM与ModelCoordinator集成.md](LLM与ModelCoordinator集成.md)、[适配器-LLM.md](适配器-LLM.md)。
...
详见 [TTS与MeloTTS集成讨论总结.md](TTS与MeloTTS集成讨论总结.md)、[适配器-TTS.md](适配器-TTS.md)。
...
| [TTS与MeloTTS集成讨论总结.md](TTS与MeloTTS集成讨论总结.md) | TTS 配置与验收 |
| [YOLO与SCRFD问题排查记录.md](YOLO与SCRFD问题排查记录.md) | 路径与拓扑排障 |
| [错误修复调试说明.md](错误修复调试说明.md) | 崩溃与退出 |
| [讨论备忘-多槽与模型演进.md](讨论备忘-多槽与模型演进.md) | 未来演进 |
