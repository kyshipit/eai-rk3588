...
- `YOU>` 后正式回答（FastAck 短反馈 + TtsPlanner 规划 → `EnqueueSentence`）
...
| `tts_planner.*` | 中文/英文正式回答规划；避免 word 级切分 |
| `tts_worker.*` | FastAck / FormalAnswer；合成/播放双线程；`generation_` 抢占 |
...
> 过渡期：`tts_stream_buffer.*` 仍承担 ingress/规划职责，待 TtsIngress/TtsPlanner 落地后移除。
